#include<stdio.h>
//面试官：请证明快指针是如何追上慢指针的 如果慢指针一次走一步 快指针一次走三步 四步行不行 n步呢？
//为什么快指针一定要走一步呢？
//答案：有的时候必须是慢指针走一步 快指针走两步 极端情况如果是三步 四步 n步的话 可能是死循环 走不出来
//如果fast一次走三步 那么距离为每次缩小2 走四步 就缩小3 走n步 就是缩小n-1
//如果某种极端的情况下 就刚好会错过 就不能相遇 假设这个环
//这个环又比较巧合 所以就一直死循环了 如过一次走n步 也可能出现这种情况
